...
```
Rbinom = Random binomial distribution. The function rbinom generates a vector of binomial distributed random variables given a vector length n, number of trials (size) and probability of success on each trial (prob)
...
```
Dbinom = defining a interval for x. Probability density function: Put simply, dbinom finds the probability of getting a certain number of successes (x) in a certain number of trials (size) where the probability of success on each trial is fixed (prob)

dbinom(x, size, prob) 


### Difference between two distributions. 
treatment A has a E(X) = 10 with se = 1.5
treatment B has a E(X) = 5 with se = 2
...
```{r}
#1 Point estimate: M1 - M2 (10-5)
10-5

#2 uncertainty surrounding the point estimate difference: finding SE from SE and Mean for each treatment (sqrt(mean1^2 + mean2^2))
...
sample(c("friday","saturday", "sunday"), size = 10, replace = TRUE, prob = c(0.55,0.35,0.1))
...
Fixing the mistake
```{r}
test_function2 <- function(n1,n2,n3){
